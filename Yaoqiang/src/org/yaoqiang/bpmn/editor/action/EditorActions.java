...
public class EditorActions extends AbstractAction {
	...
	public static EditorActions getLocaleAction(String locale) {
		return new EditorActions(LOCALE).setStringValue(locale);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (type == UNDO) {
			...
			boolean enabled = !graphComponent.isGridVisible();
			graph.setGridEnabled(enabled ? Constants.SETTINGS.getProperty("snapToGrid", "0").equals("1") : enabled);
			editor.getOrgGraphComponent().getGraph().setGridEnabled(enabled ? Constants.SETTINGS.getProperty("snapToGrid", "0").equals("1") : enabled);
			graphComponent.setGridVisible(enabled);
			for (GraphComponent com : editor.getGraphComponents().values()) {
				com.setGridVisible(enabled);
			}
			editor.getOrgGraphComponent().setGridVisible(enabled);
			...
			editor.getOrgGraphComponent().setGridStyle((int) doubleValue);
			editor.getCurrentGraphComponent().repaint();
			...
			if (newColor != null) {
				...
				editor.getOrgGraphComponent().setGridColor(newColor);
				editor.getCurrentGraphComponent().repaint();
			}
		} else if (type == GRID_SIZE) {
			double size = 0;
			String value = (String) JOptionPane.showInputDialog(graphComponent, mxResources.get("value"), mxResources.get("gridSize"), JOptionPane.PLAIN_MESSAGE,
			...
			if (size > 0) {
				...
				editor.getOrgGraphComponent().getGraph().setGridSize((int) size);
				editor.getCurrentGraphComponent().repaint();
			}
			...
			Utils.saveToConfigureFile("Locale", stringValue);
			editor.restart();
		}
