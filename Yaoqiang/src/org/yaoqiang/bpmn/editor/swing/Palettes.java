...
public class Palettes extends JPanel {
	...
	public void setPreferredCols(int count, int extra) {
		...
		if (getName().equals(mxResources.get("fragments"))) {
			int height = 80 + extra;
	...
	public void addPatternTemplate(int num) {
		...
		mxCell cell = new mxCell(mxResources.get(name + "desc"), new mxGeometry(0, 0, icon.getIconWidth(), icon.getIconHeight()), style);
		cell.setVertex(true);
		addTemplate(name, icon, cell);
	}
	...
	public void addOrganizationTemplate(final String name, ImageIcon icon, String style, Object value) {
		String width = Constants.SETTINGS.getProperty("style_" + style + "_width", "32");
		String height = Constants.SETTINGS.getProperty("style_" + style + "_height", "32");
		mxCell cell = GraphUtils.generateOrganizationCell(style, (String) value);
		cell.setGeometry(new mxGeometry(0, 0, Integer.parseInt(width), Integer.parseInt(height)));
		addTemplate(name, icon, cell);
	}
	...
	public void addTemplate(final String name, ImageIcon icon, final mxCell cell) {
		...
		entry.setText(mxResources.get(name));
		...
		entry.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (Palettes.this.getName().equals(mxResources.get("fragments")) && cell.getStyle().startsWith("fragment=") && e.isPopupTrigger()) {
					final JLabel label = (JLabel) e.getSource();
					JPopupMenu popup = new JPopupMenu();
					JMenuItem item = new JMenuItem(mxResources.get("delete"));
