...
public class BPMNViewComponent extends JPanel {
	...
	private JCheckBox caseCB = new JCheckBox(mxResources.get("case"));
	private JCheckBox wordCB = new JCheckBox(mxResources.get("word"));
	private JCheckBox regexCB = new JCheckBox(mxResources.get("regex"));
	...
	protected Component createSearchPanel() {
		...
		JLabel jl = new JLabel(mxResources.get("find") + ": ");
		...
		JButton prevBtn = new JButton(mxResources.get("previous"));
		prevBtn.setToolTipText(mxResources.get("previous"));

		final JButton nextBtn = new JButton(mxResources.get("next"));
	...
	protected void findString(boolean forward) {
		...
		if (!found) {
			...
			JOptionPane.showMessageDialog(null, mxResources.get("searchFinished"), mxResources.get("optionTitle"), JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	public void refreshView(Graph graph) {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, e.toString(), mxResources.get("error"), JOptionPane.ERROR_MESSAGE);
		}
		bpmnTextArea.setText(toDisp);
		bpmnTextArea.setCaretPosition(0);
	}
