...
public class TabbedPane extends JTabbedPane implements MouseListener, ChangeListener, ActionListener {
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getID() == MouseEvent.MOUSE_CLICKED && e.getClickCount() == 2) {
			...
			JPanel rightPane = (JPanel) editor.getMainPane().getRightComponent();
			if (maximized) {
				mainPane.setDividerLocation(mainPane.getLastDividerLocation());
				if (rightComCount == 3) {
					rightPane.add(editor.getSimulationPane(), BorderLayout.EAST);
				}
				maximized = false;
			} else {
				rightComCount = rightPane.getComponentCount();
				mainPane.setDividerLocation(0);
				rightPane.remove(editor.getSimulationPane());
				maximized = true;
			}
		}
	}
		...
		if (e.isPopupTrigger()) {
			...
			if (c instanceof mxGraphComponent) {
				...
				if (getEditor(e).getGraphComponent() == c) {
					popup.add(getEditor(e).bind(mxResources.get("editDiagramName"), ModelActions.getAction(ModelActions.DIAGRAM_NAME),
							"/org/yaoqiang/bpmn/editor/images/edit.png"));
				} else if (getEditor(e).getGraphComponents().containsValue(c)) {
					popup.add(getEditor(e).bind(mxResources.get("close"), EditorActions.getCloseCalledProcessAction(c.getName()),
