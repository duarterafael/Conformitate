...
public class NamespacePanel extends XMLPanel {
	...
	public NamespacePanel(PanelContainer pc, Definitions owner, Map.Entry<?, ?> el) {
		super(pc, owner);
	...
	public void saveObjects() {
		if (locationPanel.getText().length() != 0 && namePanel.getText().length() != 0) {
			if (((Definitions) owner).getNamespaces().containsValue(namePanel.getText())) {
				JOptionPane.showMessageDialog(null, "An Item with the name '" + namePanel.getText() + "' already exists. Specify a different name.", mxResources.get("Warning"), JOptionPane.WARNING_MESSAGE);
				return;
			}
