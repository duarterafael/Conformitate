...
public class EventDefinitionPanel extends XMLPanel {
	...
	public EventDefinitionPanel(final PanelContainer pc, final EventDefinition owner, final boolean hasRef) {
		super(pc, owner);
			...
			btn = createBuilderButton(new AbstractAction("expressionBuilder") {
				...
				public void actionPerformed(ActionEvent ae) {
					if (((Expression) typePanel.getSelectedItem()).toName().equals("timeDate")) {
						...
						BPMNEditor.getInstance().createDialog().initDialog().editBPMNElement(source, typePanel.getSelectedItem());
					}
				}
			});
