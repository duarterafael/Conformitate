...
import org.yaoqiang.bpmn.editor.dialog.panels.PrePostConditionPanel;
...
public class PanelFactory {
	...
	public PanelFactory(PanelContainer pc) {
	...
	public Panel getPanel(Object el, String type) {
		if ("predefinedProp".equals(type)) {
			return new PredefinedPropertyPanel(getPanelContainer(), (BaseElement) el);
...
//			PrePostConditionPanel prePostConditionPanel =  new PrePostConditionPanel(null);
...
//			return new ResourceAssignmentPanel(getPanelContainer(), (ResourceAssignmentExpression) el);
			...
			if ("categories".equals(type)) {
				...
				return new XMLTablePanel(getPanelContainer(), (Definitions) el, "namespace", null, columnsToShow, ((Definitions) el).getNamespaces(), 450, 150,
						true, false);
			} else if ("saveToNativeRepo".equals(type)) {
				return new SaveToNativeRepositoryPanel(getPanelContainer(), (Definitions) el);
			} else if ("versionHistory".equals(type)) {
				return new VersionHistoryPanel(getPanelContainer(), (Definitions) el);
			} else {
...
//			columnsToShow.add("resourceRef");
		...
		} else if (el instanceof ItemAwareElement && !"predefinedProp".equals(type)) {
			...
			return new NamespacePanel(getPanelContainer(), BPMNEditor.getInstance().getGraphComponent().getGraph().getBpmnModel(), (Entry<?, ?>) el);
		} else if ("deployProcessDefinition".equals(type)) {
			return new DeploymentPanel(panelContainer, (BPMNEditor) el);
		} else if ("retrieveProcessDefinitionList".equals(type)) {
			return new RetrieveDefinitionsPanel(panelContainer, (BPMNEditor) el);
		} else if ("engineConnections".equals(type)) {
			return new EngineConnectionsPanel(panelContainer, (BPMNEditor) el);
		} else if ("connection".equals(type)) {
			return new ConnectionPanel(panelContainer, (JSONObject) el);
		} else if ("organizations".equals(type)) {
			return new OrganizationsPanel(panelContainer, (BPMNEditor) el);
		} else if ("organization".equals(type)) {
			return new OrganizationPanel(panelContainer, (JSONObject) el);
		} else if ("ldapConnections".equals(type)) {
			return new LdapConnectionsPanel(panelContainer, (BPMNEditor) el);
		} else if ("ldapConnection".equals(type)) {
			return new LdapConnectionPanel(panelContainer, (JSONObject) el);
		} else if ("ldap".equals(type)) {
			if (el instanceof BPMNEditor) {
				return new LdapPanel(panelContainer, (BPMNEditor) el);
			}
		} else if ("ldapEntry".equals(type)) {
			return new LdapEntryPanel(panelContainer, (JSONObject) el);
		} else if ("searchLdap".equals(type)) {
			return new SearchLdapPanel(panelContainer, (BPMNEditor) el);
		} else if ("connectLdap".equals(type)) {
			return new ConnectLdapPanel(panelContainer, (BPMNEditor) el);
		} else if ("selectMembers".equals(type) || "selectRoleOccupants".equals(type)) {
			return new MembersPanel(panelContainer, (JSONObject) el);
		} else if (el instanceof BPMNEditor) {
			if ("selectPerformer".equals(type)) {
				return new SelectPerformerPanel(panelContainer, (BPMNEditor) el);
			}
		} else if (el == null) {
			if ("elementStyles".equals(type)) {
				return new ElementStylesPanel(panelContainer, null);
			} else if ("preferences".equals(type)) {
				return new PreferencesPanel(panelContainer, null);
			}
		}
