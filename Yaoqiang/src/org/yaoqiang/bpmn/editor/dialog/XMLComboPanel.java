...
public class XMLComboPanel extends XMLPanel {
	...
	public XMLComboPanel(PanelContainer pc, XMLElement owner, String title, List<?> choices, boolean hasEmpty, boolean isEditable, boolean isEnabled) {
		...
		JLabel jl = new JLabel(mxResources.get(title == null ? getOwner().toName() : title) + ": ");
		...
		if (owner instanceof XMLComplexChoice) {
			...
			if (getOwner().toName().equals("method") || getOwner().toName().equals("implementation")) {
				...
				if (sel.length() == 0) {
					...
					|| getOwner().toName().equals("operationRef") || getOwner().toName().equals("activityRef") || getOwner().toName().equals("yaoqiang:value")) {
				if (sel.length() == 0) {
					chsn = new XMLElementView(new XMLEmptyElement(), XMLElementView.TONAME);
				} else {
					...
					} else if (getOwner().toName().equals("yaoqiang:value")) {
						FlowElements container = (FlowElements) owner.getParent().getParent();
						XMLElement el = container.getFlowElement(sel);
						if (el == null) {
							chsn = new XMLElementView(sel);
						} else {
							chsn = new XMLElementView(el, XMLElementView.TONAME);
						}
					} else {
						XMLElement selEl = defs.getRootElement(sel);
						if (selEl == null) {
			...
			chsn = new XMLElementView(mxResources.get(owner == null ? "" : getOwner().toName()));
		}
	...
	public void saveObjects() {
		...
		if (owner instanceof XMLComplexChoice) {
			...
			if (getOwner().toName().equals("type") && getOwner().getParent() instanceof ResourceParameter || getOwner().toName().equals("itemSubjectRef")
					...
					|| getOwner().toName().equals("targetRef") || getOwner().toName().equals("sourceRef") || getOwner().toName().equals("yaoqiang:value")) {
	...
	public List<XMLElementView> convertToXMLElementViewList(Collection<?> c, boolean hasEmpty) {
		...
		while (it.hasNext()) {
			...
			if (next instanceof XMLElement) {
				...
				vec.add(new XMLElementView(mxResources.get((String) next)));
			}
		}
		return vec;
	}
